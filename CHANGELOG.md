...
## [v1.7.3-perf.1] - 2026-09-15

### ⚡ 唤出延迟实测调优 & 渲染热路径全线池化 (Measured Latency Tuning & Hot-Path Pooling)

1. **DragThreshold 拖动触发距离实测调优 (Measured DragThreshold Tuning)**：
   - **数据驱动决策**：通过 `WheelLatency` 全链路埋点采集 111 次真实手势会话，测得按下→呈现中位约 74ms，其中程序侧（Present→首帧）中位仅约 7ms —— 延迟预算的大头是用户拖动越过阈值的时间，而非程序渲染；
   - **默认值调优**：拖动触发距离默认值 25px → **18px**，按典型手势滑动速度折算按下→呈现中位缩短约 20~25ms，同时保留充分防抖余量，杜绝轻微手部漂移导致的误唤出；
   - **无损迁移**：配置加载时自动将仍停留在旧默认 25px 与调优过渡值 15px 的配置迁移至新默认 18px 并回写；用户手动调过的自定义值不受任何影响。

2. **渲染热路径全线池化与缓存 (Renderer Hot-Path Pooling & Caching)**：
   - **Effect 冻结共享**：五大渲染器（液态毛玻璃/经典圆弧/极简扇区/猫爪/基础类）高光光晕、常态阴影、中心退出光晕等 Effect 全部改为冻结共享实例，高亮切换从「实例化 Effect + GPU 资源重建」降为一次引用赋值；
   - **子环视觉跨手势缓存**：二级子环视觉元素跨手势常驻画布，唤出时直接复用本地缓存，消除每手势首次触碰子扇区的视觉树全量重建与 GC 抖动；全展开模式改为按扇区切片缓存，根除累计快照跨手势复制导致的命中错位隐患；
   - **高亮增量更新**：子环高亮从「每帧全遍历 N 个子扇区」改为「恢复旧高亮 + 仅点亮新扇区」两段式，每次划过仅触碰 2 个元素；扇区文字/图标元素引用构建时直查缓存，消除每次高亮的视觉树 LINQ 扫描（12 扇区 + 全子环展开场景每次手势约消除百余次遍历）；
   - **动画对象复用**：CubicEase/BackEase/QuadraticEase/CircleEase 等缓动模板静态池化，杜绝每手势堆分配。

3. **钩子线程零分配与调度优先级提升 (Hook-Thread Zero-Allocation & Dispatch Priority)**：
   - **手势提示缓存**：图样预览与提示文本按输入状态缓存，钩子线程每次鼠标采样（每手势 100+ 次）在输入未变化时完全跳过列表遍历、字符串拼接与映射表扫描；
   - **轨迹浮层按需重排**：提示文本未变化时跳过重复 `Measure` 强制布局，缓存尺寸复用，轨迹隐藏时重置；
   - **调度优先级提升**：手势轨迹与提示的 UI 调度优先级从 `Background`（最低档，易被布局/渲染队列饿死）提升至 `Input` 档，轨迹跟手性显著改善。

4. **控制台渲染资源瘦身与打开时序重塑 (Settings Console Slimming & Open-Sequence Restructure)**：
   - 设置控制台 XAML 中 34 处逐字节重复的内联 `DropShadowEffect` 合并为 3 个冻结共享资源（卡片阴影/软芯片阴影/芯片阴影），视觉输出逐像素一致，解析即省下 34 份独立 GPU 模糊资源；
   - **打开时序重塑**：实测埋点显示控制台打开约 1.3s，其中数据绑定初始化约 0.8s、XAML 解析约 0.33s。现将重量级数据绑定从构造函数移出至首帧渲染之后（`ContentRendered` → `ApplicationIdle`），窗口空壳于约 0.4s 即完成上屏并可交互，内容随后无感填充；请求页签暂存于 `_pendingTabRequest` 待初始化完成后消费，普通启动与静默启动路径行为一致；
   - 控制台关闭 30 秒延迟释放路径实测验证：工作集回落至约 31.6MB，稳居 15~30MB 后台驻留基准。
